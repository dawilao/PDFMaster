...
import os, errno, requests, webbrowser, customtkinter, shutil, re
...
def versão():
    instrucoes = (
        ...
        "O PDFMaster foi desenvolvido para facilitar a criação e manipulação de arquivos PDF. Ele oferece ferramentas para gerar PDFs a partir de imagens e compactar/dividir PDFs, otimizando esses processos.\n"
        "\nFunções do programa:"
        "\n    1. Imagem para PDF: Converte imagens JPG de uma pasta especificada em um arquivo PDF."
        "\n    2. Dividir PDF: Separa as páginas de um PDF selecionado em arquivos individuais."
        "\n    3. Dividir PDF por Tamanho: Otimiza e divide um PDF selecionado em arquivos menores, com no máximo 5 MB cada."
        )

    # Exibe uma mensagem com informações sobre a versão do programa
    messagebox.showinfo("Sobre o Programa", 
                        f"{instrucoes}\n\nPrograma criado por {__author__}\nVersão {__version__}\nUltima alteração em 14/11/2024, às 11:00")

# Função para mostrar e esconder os botões de "Versão" e "Contate o programador"
def toggle_ajuda():
    if btn_versao.winfo_ismapped():
        btn_versao.grid_forget()
        btn_contato_programador.grid_forget()
    else:
        # Adiciona os botões "Versão" e "Contate o programador" ao lado do botão "Sair"
...
def convert_to_pdf(image_folder, output_pdf):
    ...
    arquivo = os.path.join(image_folder, "RELATÓRIO FOTOGRÁFICO.pdf")
    ...
    # Verifica se no diretório já existe arquivo PDF de mesmo nome
...
def dividir_por_tamanho(caminho, caminho_saida, tamanho_mb_maximo=4.4):
